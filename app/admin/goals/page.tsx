        ...
        <div className="right-panel">
          ...
          {selectedInvestor && (
            <>
              ...
              {/* Summary strip */}
              {goals.length > 0 && (() => {
                // ✅ Use same weighted corpus formula as the PDF
                const uiWeights = goals.map(g =>
                  (parseFloat(String(g.target_amount)) || 0) / Math.max(1, g.target_year - currentYear)
                );
                const uiTotalWeight = uiWeights.reduce((s, w) => s + w, 0) || 1;
                ...
                // ✅ FIX: Actual shortfall using weighted corpus per goal
                const actualShortfall = goals.reduce((s, g, gi) => {
                  const yrs = Math.max(1, g.target_year - currentYear);
                  const fv = calcFutureValue(g.target_amount, g.inflation_rate, yrs);
                  const corpus = totalInvested * (uiWeights[gi] / uiTotalWeight);
                  const fvCorpus = calcFutureValue(corpus, g.expected_return, yrs);
                  return s + Math.max(0, fv - fvCorpus);
                }, 0);

                // ✅ FIX: SIP using weighted corpus, not 0
                const totalSIP = goals.reduce((s, g, gi) => {
                  const yrs = Math.max(1, g.target_year - currentYear);
                  const fv = calcFutureValue(g.target_amount, g.inflation_rate, yrs);
                  const corpus = totalInvested * (uiWeights[gi] / uiTotalWeight);
                ...
                return (
                  <div className="summary-strip">
                    ...
                    <div className="sum-card">
                      ...
                      <div className="sum-value red-val">{formatINR(actualShortfall)}</div>
                    </div>
                    <div className="sum-card">
              ...
              {!goalsLoading && goals.length > 0 && (
                <div className="goals-grid">
                  {goals.map((goal: any, goalIdx: number) => {
                    ...
                    // ✅ FIX: Use same weighted corpus formula as PDF so UI matches report
                    const cardWeights = goals.map((g2: any) =>
                      (parseFloat(String(g2.target_amount)) || 0) / Math.max(1, g2.target_year - currentYear)
                    );
                    const cardTotalWeight = cardWeights.reduce((s: number, w: number) => s + w, 0) || 1;
                    const corpusForCard = totalInvested * (cardWeights[goalIdx] / cardTotalWeight);
                    const fvCorpusForCard = calcFutureValue(corpusForCard, goal.expected_return, yrs);

                    const sip = calcRequiredSIP(fv, corpusForCard, goal.expected_return, yrs);
                    const shortfall = Math.max(0, fv - fvCorpusForCard);
                    const pct = Math.min(100, (fvCorpusForCard / fv) * 100);
